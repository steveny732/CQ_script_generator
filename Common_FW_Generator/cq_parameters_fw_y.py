...
#!/usr/bin/python
#
# This is derived from a cadquery script for generating FW Y models in X3D format.
#
...
# import importlib
# print("Import cq_parameters_fw_y.py")
# import Params
...
pkg_defs = {
    'DEV_4_pkg': Pkg_params(                    # from http://www.???
    ...
    modelName = 'DEV_4_var',                # modelName
    rotation = 0,                           # rotation if required
    dest_dir_prefix = '/FW_y'               # destination sub directory
    ),
    'DEV_5_pkg': Pkg_params(                    # from http://www.???
    the = 9.0,                              # body angle in degrees
    tb_s = 0.15,                            # top part of body is that much smaller
    c = 0.1,                                # pin thickness, body center part height
    R1 = 0.1,                               # pin upper corner, inner radius
    ...
    pins_color_key = "metal grey pins",     # pins color
    mark_color_key = "light brown label",   # marker color
    modelName = 'DEV_5_type',               # modelName
    rotation = 0,                           # rotation if required
    dest_dir_prefix = '/FW_y'               # destination sub directory
    ),
    'DEV_6_pkg': Pkg_params(                    # from http://www.???
    the = 9.0,                              # body angle in degrees
    tb_s = 0.15,                            # top part of body is that much smaller
    c = 0.1,                                # pin thickness, body center part height
    R1 = 0.1,                               # pin upper corner, inner radius
    ...
    pins_color_key = "metal grey pins",     # pins color
    mark_color_key = "light brown label",   # marker color
    modelName = 'DEV_6_package',            # modelName
    rotation = 0,                           # rotation if required
    dest_dir_prefix = '/FW_y'               # destination sub directory
    ),
}